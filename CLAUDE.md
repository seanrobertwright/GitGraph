...
- **Untracked-file hygiene at branch-cut.** Before opening an implementation PR, `git status` on the feature branch must show no untracked files outside `.agents/`. Local-tool scratch dirs (`.kilocode/`, `.pi/`, `.qoder/`, `.agents/skills/`, `skills-lock.json`, etc.) must be added to `.gitignore` *before* the PR opens, never after — the GitHub squash UI will otherwise sweep them in alongside the implementation. The pre-PR check is: every path in `git diff --name-only main...HEAD` matches the plan's "Primary Systems Affected" or is an explicitly planned new file. Anything else is a sweep-in to triage before merging. Phase 4 was the second occurrence of this class — automate it as a checklist, not a re-read of this rule.
- **Plan-prescribed spike artifacts.** When a plan prescribes a throwaway spike (e.g. `_spike/` page, `_spike-*.spec.ts`), execute it as written — do not inline the measurement into the production tree. The spike's value is not just the number it produces; it's the recorded measurement run and the clean separation from production code. The measurement value gets recorded in the plan body itself (in the relevant downstream task, with a comment citing the spike) before the spike code is deleted. If the plan recipe genuinely doesn't fit the situation, append a "Post-execution corrections" section to the plan rather than silently substituting.
